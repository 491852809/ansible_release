app.controller('Sound2_Ctrl', 
   function ($scope,$timeout,Music,angularPlayer) {
    ...
    Music.query(function(musics){             //put on where values of query is going to deal with ;
        allmusic = [];
        for (i in musics){                                  //aquire musiclist array
            music_src = musics[i].music_file
            img_src = musics[i].music_img
            owner_src = musics[i].username
            console.log(music_src.split('/')[5]);
            allmusic.push(
                {
                    id: i,
                    owner: owner_src,
                    url: music_src, 
                    title: decodeURI(music_src.split('/')[5].split('.')[0]),
                    img: decodeURI(img_src),
                    img_title: decodeURI(img_src.split('/')[5].split('.')[0]),
                    artist: 'present no use'
                });
            };
    
    $scope.music_owner = allmusic[0].owner;                                  //change the musiclist of owner;
    ...
    $scope.$watch('music_owner',function() {
        ...
        for (i in allmusic){                             //choose the musiclist via owner;
            if (allmusic[i].owner == $scope.music_owner){
                musiclist.push(allmusic[i]);
            };
        };

        for (i in musiclist){                                       // fix user musiclist id;
            musiclist[i].id=i;                                     
            console.log(musiclist[i]);
        };

        $scope.songs = musiclist;
        console.log($scope.songs);
        $scope.list_num = parseInt(musiclist[musiclist.length-1].id)+1;
        $scope.person_num = parseInt(musiclist.length);
        // console.log($scope.list_num,$scope.person_num);
        ...
        $timeout(function(){
            angularPlayer.clearPlaylist(function (data) {
                ...
                angularPlayer.play();
                angularPlayer.pause();
                if(changeInput.value != undefined){      // keep the volume when change the user 
    ...
    changeInput.onchange = function() {
        angularPlayer.adjustVolume(changeInput.value);
    ...
    $scope.$on('track:progress', function(event, data) {
        if ( (parseInt(data) === 100 || parseInt(data) === 99) && $scope.replay == true) {
